...
class BollingerStrategy {
  ...
  calculateRSI(closes, period = 14) {
    ...
    // Sonraki değerler için smooth average kullanın:
  ...
  async generateSignal(candles, symbol) {
    ...
    const { bb } = indicators;
    const bbBasis = bb ? bb.basis : 'N/A'; 
    
    if (!bb || typeof bb !== 'object') {
      logger.error(`Invalid Bollinger Bands data for ${symbol}`);
      return { signal: 'NEUTRAL' };
    }
...
`);
    ...
    if (weakBuyConditions) {
      const unmetConditions = buyConditionNames
        .map((conditionName, index) => {
          if (!strongBuyConditions[index]) {
            // Koşulun o anki değerini hesapla
            let currentValue = '';
            switch (conditionName) {
              case "Price Below Lower Bollinger Band":
                currentValue = `Current Price: ${lastClose}, Lower BB: ${bb.lower}`;
                break;
              case "Stochastic K Below 25":
